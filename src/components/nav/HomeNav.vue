...
import {listItem, listAdvertise} from '@/api/navigation'
...
export default {
  ...
  methods: {
    getItemList () {
      const itemNavList = localStorage.getItem('itemList')
      if (itemNavList) {
        this.itemList = JSON.parse(itemNavList)
      } else {
    ...
    getHomeAdvertiseList () {
      listAdvertise().then(response => {
        this.rotateChartList = response.data.rotateChartList
